...
namespace Monappoly_ASP.Pages.Objects.Boards.Spaces.Card;
...
public class Edit : PageModel
{
    ...
    public class CardSpaceInputModel
    {
        public string Name { get; set; }
        [DisplayName("Card Type")]
        public int CardTypeId { get; set; }

        public CardSpaceInputModel()
        {
        }

        public CardSpaceInputModel(CardBoardSpace space)
        {
            Name = space.Name;
            CardTypeId = space.CardTypeId;
        }
        
        public void FillSpace(CardBoardSpace space)
        {
            space.Name = Name;
            space.CardTypeId = CardTypeId;
        }
    }
    
    [BindProperty]
    public CardSpaceInputModel Input { get; set; }
    ...
    public async Task<IActionResult?> SetupPage(int boardId, int index)
    {
        ...
        CardTypes = (await _cardService.GetTypes()).Select(t => new SelectListItem
        {
            Text = t.Name,
            Value = t.Id.ToString()
        }).ToList();
    ...
    public async Task<IActionResult> OnGet(int boardId, int id, int index, bool replace = false)
    {
        ...
        if (Adding || Replacing)
        {
            Input = new CardSpaceInputModel();
            return Page();
        }

        var space = await _boardSpaceService.FindCardSpace(id);
        if (space == null) return new NotFoundResult();
        
        Input = new CardSpaceInputModel(space);
        return Page();
    }
    ...
    public async Task<IActionResult> OnPost(int boardId, int id, int index, bool replace = false)
    {
        ...
        if (Adding)
        {
            ...
            res = await _boardSpaceService.TryAddCardSpace(space, ModelState);
        }
        else if (Replacing)
        {
            var space = new CardBoardSpace
        ...
        else
        {
            ...
            Input.FillSpace(space);
            res = await _boardSpaceService.TryUpdateCardSpace(space, ModelState);
        }
